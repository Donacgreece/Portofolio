...
const locales = {
  en: {
    ...
    home: "Portfolio", all: "All articles", search: "Search articles", empty: "No articles match your search.", allCategories: "All", categoryFilter: "Filter by category",
  ...
  gr: {
    ...
    home: "Portfolio", all: "Όλα τα άρθρα", search: "Αναζήτηση άρθρων", empty: "Δεν βρέθηκαν άρθρα για αυτή την αναζήτηση.", allCategories: "Όλα", categoryFilter: "Φίλτρο κατηγορίας",
...
function formatDate(date, locale) { return new Intl.DateTimeFormat(locale, { day: "numeric", month: "long", year: "numeric", timeZone: "UTC" }).format(dateValue(date)); }
...
.blog-main{padding:52px 0 88px}.blog-tools{flex-wrap:wrap;margin-bottom:22px}.category-filters{display:flex;flex-wrap:wrap;gap:8px;width:100%;padding-top:2px}.category-filter{min-height:36px;padding:0 13px;border:1px solid var(--line);border-radius:999px;color:var(--muted);background:var(--surface);font:700 10px var(--mono);cursor:pointer;transition:.18s ease}.category-filter:hover,.category-filter.active{border-color:var(--brand);color:#fff;background:var(--brand)}.post-grid{grid-template-columns:repeat(3,minmax(0,1fr));gap:18px}.post-card,.post-card.featured{grid-column:auto;min-height:0;padding:22px}.post-card[hidden]{display:none!important}.post-card h3,.featured h3{margin:20px 0 12px;font-size:clamp(23px,2.4vw,32px);line-height:1.05}.post-card p,.featured p{margin-bottom:22px;font-size:14px}.post-cover{width:calc(100% + 44px);height:170px;margin:-22px -22px 20px}.post-tag-more{border:1px solid currentColor;background:transparent;opacity:.78}.post-link{margin-top:20px}@media(max-width:1020px){.post-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:760px){.post-grid{grid-template-columns:1fr}.post-cover{height:190px}}
.blog-brand{display:inline-flex;align-items:center;gap:11px;font-weight:800;letter-spacing:-.025em}.blog-brand-logo{display:block;width:40px;height:40px;flex:0 0 auto}@media(max-width:620px){.blog-brand-name{display:none}}
.blog-nav{z-index:100;border-bottom:1px solid rgba(36,39,201,.12);background:rgba(245,245,241,.96);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px)}.blog-nav .shell{width:min(calc(100% - 48px),1320px);min-height:74px;display:grid;grid-template-columns:auto minmax(0,1fr) auto;align-items:center;gap:28px}.blog-brand{white-space:nowrap}.blog-links{grid-column:3;justify-self:end;gap:9px}.blog-button{min-height:42px;padding:0 16px;border-radius:9px;font:750 13px/1 var(--sans)}@media(max-width:760px){.blog-nav .shell{width:min(calc(100% - 24px),1540px);min-height:68px;gap:10px}.blog-brand-name{display:none}.blog-links{gap:9px}.blog-button{padding:0 13px}.blog-links .rss-link{display:none}}
</style>`;
}
...
function navigation(copy, depth, article = false, languageTarget = null) {
  const root = "../".repeat(depth);
  const home = copy.lang === "el" ? `${root}gr/` : root;
  const other = languageTarget || (copy.lang === "en" ? `${article ? "../../" : "../"}gr/blog/` : `${article ? "../../../" : "../../"}blog/`);
  const rss = copy.lang === "en" ? `${article ? "../../" : "../"}rss.xml` : `${article ? "../../" : "../"}rss.xml`;
  return `<nav class="blog-nav"><div class="shell"><a class="blog-brand" href="${home}" aria-label="Dimitris Galatsanos portfolio"><svg class="blog-brand-logo" viewBox="0 0 64 64" role="img" aria-label="DG logo" xmlns="http://www.w3.org/2000/svg"><rect width="64" height="64" rx="8" fill="#f8f8f6"/><path d="M14 8h36v6h6v36h-6v6H14v-6H8V14h6V8Z" fill="#2024c8" opacity=".12"/><rect x="13" y="13" width="38" height="38" fill="#fff" stroke="#2024c8" stroke-width="4"/><path d="M9 17V9h30" fill="none" stroke="#2024c8" stroke-width="3"/><path d="M55 47v8H25" fill="none" stroke="#2024c8" stroke-width="3"/><text x="32" y="39" text-anchor="middle" font-family="Arial,Helvetica,sans-serif" font-size="19" font-weight="700" fill="#2024c8">DG</text></svg><span class="blog-brand-name">Dimitris Galatsanos</span></a><div class="blog-links"><a class="blog-button rss-link" href="${rss}">${copy.rss}</a><a class="blog-button" href="${other}" data-blog-language>${copy.language}</a><a class="blog-button primary" href="${home}">${copy.home}</a></div></div></nav>`;
}

function languagePositionScript() {
  return `<script>(()=>{const key='portfolio-blog-language-position';const currentLanguage=document.documentElement.lang==='el'?'el':'en';localStorage.setItem('portfolio-language',currentLanguage);let saved=null;try{saved=JSON.parse(sessionStorage.getItem(key)||'null')}catch{}sessionStorage.removeItem(key);const restore=()=>{if(!saved||Date.now()-saved.createdAt>15000)return;const max=Math.max(0,document.documentElement.scrollHeight-innerHeight);scrollTo({top:Math.max(0,Math.min(max,max*Number(saved.progress||0))),behavior:'instant'})};if(saved){history.scrollRestoration='manual';requestAnimationFrame(()=>requestAnimationFrame(restore));addEventListener('load',restore,{once:true})}document.querySelectorAll('[data-blog-language]').forEach(link=>link.addEventListener('click',()=>{const max=Math.max(1,document.documentElement.scrollHeight-innerHeight);sessionStorage.setItem(key,JSON.stringify({progress:scrollY/max,createdAt:Date.now()}));localStorage.setItem('portfolio-language',currentLanguage==='en'?'el':'en')}))})();</script>`;
}

function footer(copy) { return `<footer class="blog-footer"><div class="shell"><span>© ${new Date().getUTCFullYear()} Dimitris Galatsanos</span><span>${copy.author}</span></div></footer>${languagePositionScript()}`; }

function card(post, copy, index) {
  const visibleTags = post.tags.slice(0, 3);
  const hiddenTags = post.tags.length - visibleTags.length;
  const tags = `${visibleTags.map(tag => `<span class="post-tag">${escapeHtml(tag)}</span>`).join("")}${hiddenTags ? `<span class="post-tag post-tag-more" aria-label="${hiddenTags} more tags">+${hiddenTags}</span>` : ""}`;
  ...
  const accented = Math.floor(index / 2) % 2 === 0;
  return `<article class="post-card${accented ? " featured" : ""}" data-search="${search}" data-category="${escapeHtml(category)}">${cover}<div class="post-meta"><span class="post-category">${escapeHtml(post.category || "Technology")}</span><time datetime="${escapeHtml(post.date)}">${formatDate(post.date, copy.locale)}</time><span>${readingTime(post.body)} ${copy.minute}</span></div><h3>${escapeHtml(post.title)}</h3><p>${escapeHtml(post.excerpt)}</p><div class="post-tags">${tags}</div><a class="post-link" href="${encodeURIComponent(post.slug)}/">${copy.read} →</a></article>`;
}

function indexPage(posts, copy) {
  const canonical = `${SITE}/${copy.lang === "en" ? "blog" : "gr/blog"}/`;
  const categories = [...new Set(posts.map(post => String(post.category || "Technology")))].sort((a, b) => a.localeCompare(b, copy.locale));
  const filters = `<div class="category-filters" role="group" aria-label="${escapeHtml(copy.categoryFilter)}"><button class="category-filter active" type="button" data-category="">${escapeHtml(copy.allCategories)}</button>${categories.map(category => `<button class="category-filter" type="button" data-category="${escapeHtml(category.toLocaleLowerCase(copy.locale))}">${escapeHtml(category)}</button>`).join("")}</div>`;
  return `${head({ title: `${copy.blog} | Dimitris Galatsanos`, description: copy.intro, canonical, lang: copy.lang })}<body>${navigation(copy, copy.lang === "en" ? 1 : 2)}<header class="blog-hero"><div class="shell"><span class="eyebrow">LOG / KNOWLEDGE BASE</span><h1>${copy.blog}</h1><p>${copy.intro}</p></div></header><main class="blog-main"><div class="shell"><div class="blog-tools"><h2>${copy.latest}</h2><label><span class="sr-only">${copy.search}</span><input class="blog-search" id="blogSearch" type="search" placeholder="${copy.search}…" autocomplete="off"></label>${filters}</div><div class="post-grid" id="postGrid">${posts.map((post, index) => card(post, copy, index)).join("")}</div><div class="blog-empty" id="blogEmpty"${posts.length ? "" : ' style="display:block"'}>${copy.empty}</div></div></main>${footer(copy)}<script>const search=document.getElementById('blogSearch');const cards=[...document.querySelectorAll('.post-card')];const empty=document.getElementById('blogEmpty');const filters=[...document.querySelectorAll('.category-filter')];const normalize=value=>String(value||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLocaleLowerCase(document.documentElement.lang);let activeCategory='';function applyFilters(){const q=normalize(search.value.trim());let shown=0;cards.forEach(card=>{const match=normalize(card.dataset.search).includes(q)&&(!activeCategory||card.dataset.category===activeCategory);card.hidden=!match;if(match)shown++});empty.style.display=shown?'none':'block'}search.addEventListener('input',applyFilters);search.addEventListener('search',applyFilters);filters.forEach(button=>button.addEventListener('click',()=>{activeCategory=button.dataset.category;filters.forEach(item=>item.classList.toggle('active',item===button));applyFilters()}));</script></body></html>`;
}
...
function articlePage(post, posts, copy, translations) {
  ...
  return `${head({ title: `${post.title} | Dimitris Galatsanos`, description: post.excerpt, canonical, lang: copy.lang, type: "article", image: post.cover })}<body>${navigation(copy, copy.lang === "en" ? 2 : 3, true, languageLink)}<header class="article-hero"><div class="shell"><a class="article-back" href="../">← ${copy.back}</a><div class="post-meta"><span class="post-category">${escapeHtml(post.category || "Technology")}</span><time datetime="${escapeHtml(post.date)}">${formatDate(post.date, copy.locale)}</time><span>${readingTime(post.body)} ${copy.minute}</span></div><h1>${escapeHtml(post.title)}</h1><p class="article-lead">${escapeHtml(post.excerpt)}</p></div>${cover}</header><main class="shell article-layout"><article class="article-body">${markdown(post.body)}${suggestions.length ? `<h2>${copy.related}</h2>${suggestions.map(item => `<p><a href="../${encodeURIComponent(item.slug)}/">${escapeHtml(item.title)} →</a></p>`).join("")}` : ""}</article><aside class="article-aside"><b>${copy.author}</b><span>${formatDate(post.date, copy.locale)}</span><span>${readingTime(post.body)} ${copy.minute}</span>${languageLink ? `<a class="blog-button" href="${languageLink}" data-blog-language>${copy.language}</a>` : ""}</aside></main>${footer(copy)}</body></html>`;
}

function rss(posts, copy) {
  const base = `${SITE}/${copy.lang === "en" ? "blog" : "gr/blog"}`;
  return `<?xml version="1.0" encoding="UTF-8"?><rss version="2.0"><channel><title>${escapeHtml(copy.blog)}</title><link>${base}/</link><description>${escapeHtml(copy.intro)}</description><language>${copy.lang}</language>${posts.map(post => `<item><title>${escapeHtml(post.title)}</title><link>${base}/${encodeURIComponent(post.slug)}/</link><guid>${base}/${encodeURIComponent(post.slug)}/</guid><pubDate>${dateValue(post.date).toUTCString()}</pubDate><description>${escapeHtml(post.excerpt)}</description></item>`).join("")}</channel></rss>`;
}
...
async function loadPosts(copy) {
  ...
  return posts.sort((a, b) => dateValue(b.date).getTime() - dateValue(a.date).getTime() || b.slug.localeCompare(a.slug, copy.locale));
}
